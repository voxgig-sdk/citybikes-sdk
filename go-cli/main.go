// Command Citybikes-cli is a CLI + REPL for the Citybikes SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CITYBIKES_APIKEY (api key), CITYBIKES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CITYBIKES_APIKEY carries the API key and
	// CITYBIKES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CITYBIKES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CITYBIKES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCitybikesSDK(opts)
