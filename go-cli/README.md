...
boru-driven command-line client **and** interactive REPL for the Citybikes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/citybikes-cli)
make build
...
./citybikes-cli list network
./citybikes-cli load 1 network            # {id:1} shorthand
./citybikes-cli load '{id:1}' network       # explicit match map

# 5. Override the API base URL for a single call
CITYBIKES_BASE=https://api.example.com ./citybikes-cli list network

# 6. No arguments -> interactive REPL
./citybikes-cli
citybikes> list network
citybikes> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/citybikes-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/citybikes-cli list network
   ```
...
```sh
./citybikes-cli load 1 network          # scalar shorthand for {id:1}
./citybikes-cli load '{id:1}' network     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `citybikes>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/citybikes-cli`. |
...
(`./citybikes-cli <expr>`) and interactively (the REPL), and expressions compose the
