...
import (
	...
	omni "github.com/voxgig-sdk/citybikes-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CitybikesError); is {
				out := map[string]any{
					"name":    "CitybikesError",
...
func omniCtx(arg any, client *sdk.CitybikesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CitybikesSDK); is {
			client = live
