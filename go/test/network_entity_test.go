...
func TestNetworkEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CITYBIKES_TEST_NETWORK_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		_, networkRef01ListOk := networkRef01ListResult.([]any)
...
func networkBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CITYBIKES_TEST_NETWORK_ENTID")
	...
	if env["CITYBIKES_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewCitybikesSDK(core.ToMapAny(mergedOpts))
	}
