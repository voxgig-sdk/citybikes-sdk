...
require_relative 'Citybikes_types'
...
class CitybikesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CitybikesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CitybikesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CitybikesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CitybikesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CitybikesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CitybikesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CitybikesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => CitybikesError.new(
        "#{op}_allow",
        "CitybikesSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = CitybikesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CitybikesError.new(
        "graphql_error", "CitybikesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
