...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = CitybikesOmni.make_runner(TEST_JSON_FILE, CitybikesSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : CitybikesOmni.errify(err)
    end
