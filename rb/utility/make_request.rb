...
module CitybikesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CitybikesResponse.new({})
    ...
    if fetch_err
      ...
      response = CitybikesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CitybikesResponse.new(fetched)
    else
