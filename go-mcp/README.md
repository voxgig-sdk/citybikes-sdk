...
MCP server exposing the Citybikes SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `citybikes_list` | `entity`, optional `query` map | First page of records as JSON |
| `citybikes_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `citybikes_list` and
`citybikes_load` tools become available in new sessions.
