...
[MCP](https://modelcontextprotocol.io) server exposing the Citybikes SDK as
two agent tools — `citybikes_list` and `citybikes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/citybikes-mcp)
make build

# 2. Provide credentials via the environment
export CITYBIKES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user citybikes \
  -- /absolute/path/to/citybikes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./citybikes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// citybikes_list: first page of records
{ "entity": "network" }
{ "entity": "network", "query": { } }

// citybikes_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/citybikes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user citybikes \
     -- "$PWD"/dist/*/citybikes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `citybikes_list` and `citybikes_load` tools now appear
   in new sessions. Ask the agent to *"list network using citybikes"*
   and it calls `citybikes_list` with `{"entity":"network"}`.
...
```sh
export CITYBIKES_APIKEY=sk_live_xxx            # API key
export CITYBIKES_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `citybikes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `citybikes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CITYBIKES_APIKEY` | API key sent with every request. |
| `CITYBIKES_BASE` | Optional override of the API base URL. |
...
```sh
./citybikes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"citybikes_load","arguments":{"entity":"network","query":{"id":1}}}}'
```
