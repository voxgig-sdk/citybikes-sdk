...
class FtCtrl extends CitybikesControl
{
...
class FtClient
{
    ...
    private CitybikesUtility $utility;

    public function __construct(CitybikesUtility $utility, array $options)
    {
    ...
    public function get_utility(): CitybikesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CitybikesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CitybikesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CitybikesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CitybikesConfig::make_feature($name);
    ...
    private static function buildUrl(CitybikesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CitybikesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CitybikesResponse($fetched);
        }
    ...
    private function fail(CitybikesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CitybikesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
