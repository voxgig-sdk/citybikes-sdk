...
from projectname_sdk.core.control import CitybikesControl
...
class CitybikesContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, CitybikesSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, CitybikesResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, CitybikesResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return CitybikesError(code, msg, self)
