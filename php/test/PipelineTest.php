...
{
    private static function utility(): CitybikesUtility
    {
        return new CitybikesUtility();
    }
    ...
    private static function ctx(array $over = []): CitybikesContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CitybikesContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CitybikesOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CitybikesError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CitybikesOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CitybikesMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CitybikesOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CitybikesMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CitybikesOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CitybikesMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CitybikesSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CitybikesMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CitybikesResponse([]),
            'result' => new CitybikesResult([]),
        ]);
        [, $err] = CitybikesMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CitybikesSpec([]),
            'response' => null,
            'result' => new CitybikesResult([]),
        ]);
        [, $err] = CitybikesMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CitybikesSpec(['step' => 's']),
            'response' => new CitybikesResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CitybikesResult([]),
        ]);
        [, $err] = CitybikesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CitybikesSpec(['step' => 's']),
            'response' => new CitybikesResponse(self::resp(200, ['v' => 1])),
            'result' => new CitybikesResult([]),
        ]);
        [, $err] = CitybikesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CitybikesSpec(['step' => 's']),
            'response' => new CitybikesResponse(self::resp(200, ['v' => 2])),
            'result' => new CitybikesResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CitybikesMakeResponse::call($ctx);
    ...
    {
        $preset = new CitybikesResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CitybikesSpec([]),
            'response' => new CitybikesResponse([]),
            'result' => new CitybikesResult([]),
        ...
        [$got, $err] = CitybikesMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CitybikesResult([])]);
        [, $err] = CitybikesMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CitybikesSpec([]), 'result' => null]);
        [, $err] = CitybikesMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CitybikesSpec(['step' => 's']),
            'result' => new CitybikesResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CitybikesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CitybikesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CitybikesSpec(['step' => 's']),
            'result' => new CitybikesResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CitybikesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CitybikesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CitybikesResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CitybikesSpec([]),
            'result' => new CitybikesResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CitybikesMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CitybikesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CitybikesError('boom', 'boom');
        $utility->fetcher = function (CitybikesContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CitybikesSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CitybikesMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CitybikesContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CitybikesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CitybikesResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CitybikesContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CitybikesSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CitybikesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CitybikesResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CitybikesSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CitybikesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CitybikesMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CitybikesMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CitybikesResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CitybikesDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CitybikesResult(['ok' => false])]);
        $this->expectException(CitybikesError::class);
        CitybikesDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CitybikesResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CitybikesMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CitybikesMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CitybikesBaseFeature();
        $b = new CitybikesBaseFeature();
        CitybikesFeatureAdd::call($ctx, $a);
        CitybikesFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CitybikesBaseFeature
    {
        $f = new CitybikesBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CitybikesFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CitybikesContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CitybikesSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CitybikesPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CitybikesResponse(['headers' => null]),
            'result' => new CitybikesResult([]),
        ]);
        CitybikesResultHeaders::call($ctx);
