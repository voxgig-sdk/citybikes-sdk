...
class CitybikesNetsimFeature extends CitybikesBaseFeature
{
    ...
    private function _simulate(CitybikesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(CitybikesContext $ctx, array $applied): void
    {
        $client = $this->client;
