...
class NetworkEntity
{
    ...
    {
        if ($args) {
            $this->_data = CitybikesHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = CitybikesHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws CitybikesError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CitybikesHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param NetworkListMatch|array|null $reqmatch Match filter (any subset
     *   of Network fields) as an assoc-array; NetworkListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Network[]|array A list of Network items as assoc-arrays at
     *   the SDK boundary; throws CitybikesError on failure (item-5 convention).
     */
