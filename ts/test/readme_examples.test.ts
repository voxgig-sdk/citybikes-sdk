...
//     (\`new CitybikesSDK(...)\` / \`CitybikesSDK.test(...)\` become
//     \`CitybikesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new CitybikesSDK(...)\` and any
// \`CitybikesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CitybikesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CitybikesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
