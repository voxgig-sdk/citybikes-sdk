# Citybikes SDK test runner

from __future__ import annotations
import os
import json

from citybikes_sdk.utility.voxgig_struct import voxgig_struct as vs


class CitybikesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CitybikesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CitybikesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CitybikesTestRunner.getenv("CITYBIKES_TEST_LIVE")
        override = CitybikesTestRunner.getenv("CITYBIKES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CitybikesTestRunner.getenv(key)
        ...
        explain = CitybikesTestRunner.getenv("CITYBIKES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CITYBIKES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CitybikesTestRunner._test_control is not None:
            return CitybikesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CitybikesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CitybikesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CitybikesTestRunner.load_test_control()
...
def load_env_local():
    CitybikesTestRunner.load_env_local()


def env_override(m):
    return CitybikesTestRunner.env_override(m)


def entity_data(v):
    return CitybikesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CitybikesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CitybikesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CitybikesTestRunner.load_test_control()


def live_client_options():
    return CitybikesTestRunner.live_client_options()


def live_delay_ms():
    return CitybikesTestRunner.live_delay_ms()
