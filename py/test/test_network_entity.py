...
class TestNetworkEntity:
    ...
    def test_should_stream(self):
        ...
        base = CitybikesSDK.test(seed, None)
        seen = list(base.Network(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from citybikes_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = CitybikesSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Network(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CITYBIKES_TEST_NETWORK_ENTID JSON to run live")
        client = setup["client"]
        ...
        network_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(network_ref01_data_dt0_loaded))
...
def _network_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CITYBIKES_TEST_NETWORK_ENTID"))
    ...
    if env.get("CITYBIKES_TEST_LIVE") == "TRUE":
        ...
        client = CitybikesSDK(helpers.to_map(merged_opts))

    _live = env.get("CITYBIKES_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("CITYBIKES_TEST_EXPLAIN") == "TRUE",
        "live": _live,
